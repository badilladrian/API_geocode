...
class StreetViewer(object):
    ...
    def get_meta(self):
        ...
        meta_response = requests.get('https://maps.googleapis.com/maps/api/streetview/metadata?',
        ...
        if meta_response.ok:
            print(meta_info)
            if self.meta_status == "ZERO_RESULTS" or self.meta_status == "NOT_FOUND" or self.meta_status == "UNKNOWN_ERROR":
                return None
    ...
    def save_pic(self, meta_info):
        ...
        header_path = self.folder_directory + "header_{}.json".format(
            meta_info['pano_id'])
