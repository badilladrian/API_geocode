...
class Utils:
    def __init__(self):
        self.api_key = 'AIzaSyCL3WravFN_wNUfKU6cC4QRWAOzfbfo49g'  # need to change to values.yaml os.environ()
        gmaps.configure(api_key=self.api_key) # gmaps is to create an Dynamic HTML MAP w/ two markers pass as params
        self.google_client = googlemaps.Client(key=self.api_key) # google API address to geocode 
    ...
    def address_to_geocode(self,address):    
        ...
        geocode_result = self.google_client.geocode(address) # /STATIC/ GEOCODE_API.JSON 
